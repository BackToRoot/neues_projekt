  ...
  return (
    <>
      ...
      {!appStarted ? (
        ...
        <div
          className={`bg-slate-900 text-white min-h-[100svh] pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]
                      font-sans flex items-center justify-center overflow-hidden ${view === 'intro' ? 'p-0' : 'p-4'}`}
        >
...
function Quiz({ onCorrectAnswer }) {
  ...
  const [showIntro, setShowIntro] = React.useState(true); // <— NEU
  const [loading, setLoading] = React.useState(true);     // stabileres "Lade…"

  // Fragen vorladen, während Intro angezeigt wird
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (e2 || !all?.length) {
          setFeedback('Fehler beim Laden der Fragen.');
          setLoading(false);
          return;
        }
        const pick = (arr, n) => shuffle(arr).slice(0, Math.min(n, arr.length));
        const prepared = shuffle([
          ...pick(all.filter(q=>q.difficulty==='easy'),5),
          ...pick(all.filter(q=>q.difficulty==='hard'),5),
  ...
  // Intro-Screen vor dem Quiz
  if (showIntro) {
    return (
      <div className="w-[92vw] max-w-md mx-auto bg-slate-800/90 backdrop-blur-sm p-6 sm:p-8 rounded-xl shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          <span className="text-3xl">🕵️‍♂️</span>
          <h2 className="text-2xl font-bold">Mini-Check vor der Party</h2>
        </div>
        <p className="text-slate-300 leading-relaxed">
          Dieses kurze Quiz prüft, ob du <span className="font-semibold">bereit für die Feier</span> bist. 😉
        </p>
        <ul className="mt-4 space-y-2 text-slate-300">
          <li>• Insgesamt <span className="font-semibold">10 Fragen</span> (gemischt)</li>
          <li>• Reihenfolge &amp; Antworten sind <span className="font-semibold">zufällig</span></li>
          <li>• Bei einer <span className="font-semibold">richtigen</span> Antwort geht es zur Anmeldung</li>
        </ul>
        <button
          onClick={() => setShowIntro(false)}
          disabled={loading}
          className="mt-6 w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold py-3.5 px-4 rounded-lg transition-colors duration-300"
        >
          {loading ? 'Lade Fragen…' : 'Quiz starten'}
        </button>
        {feedback && loading && <p className="mt-3 text-center text-amber-400">{feedback}</p>}
      </div>
    );
  }

  // Während (noch) geladen wird – falls jemand das Intro superschnell wegklickt
  if (loading || !questions.length) {
    return (
      <div className="w-[92vw] max-w-md mx-auto bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl text-center">
        Lade Fragen… {feedback && <div className="mt-2 text-amber-400">{feedback}</div>}
      </div>
    );
  }
  ...
  return (
    <div className="w-[92vw] max-w-md mx-auto bg-slate-800/90 backdrop-blur-sm p-6 sm:p-8 rounded-xl shadow-2xl">
      {/* kleiner Hinweis über dem Quiz */}
      <div className="mb-3 text-sm text-slate-300 flex items-center gap-2">
        <span className="text-lg">✅</span>
        <span>Dieses Quiz checkt kurz, ob du für die Feier geeignet bist.</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {q.options.map(opt => (
          <button
            ...
            disabled={isLocked}
            className="w-full bg-slate-700 hover:bg-indigo-600 disabled:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3.5 sm:py-4 px-4 rounded-lg transition-colors duration-300"
          >
            {opt}
          </button>
        ))}
